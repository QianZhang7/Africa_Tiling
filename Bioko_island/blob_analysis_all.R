blob_analysis_plot(mcdi100_shp, pop_var = "PEOPLE_N", dataname = "MCDI 100m")

mcdi1k_shp$pop_num <- as.numeric(as.character(mcdi1k_shp$pop))
blob_analysis_plot(mcdi1k_shp, pop_var = "pop_num", dataname = "MCDI 1k")

landscan_shp <- rasterToPolygons(landscan)
landscan_shp <- landscan_shp[landscan_shp$dblbnd>0,]
blob_analysis_plot(landscan_shp, pop_var = "dblbnd", dataname = "Landscan 1k")
...
blob_analysis_plot_worldpop_1k <- function(shapefile, pop_var, dataname){
  ...
  plot(log(com_cluster,10), log(popsum,10), main = paste(dataname, ":", clu$no, "blobs (4 directions)",sep = " "),
       ylab = "Population in each blob", xlab = "Number of connected pixels in each blob", xaxt='n', yaxt='n', xlim = c(0, 4), ylim = c(0, 6))
  xticks <- seq(0, 4, by=1)
...
blob_analysis_plot_worldpop_1k(world_1k_2010_shp, pop_var = "AFR_PPP_2010_adj_v2", dataname = "WorldPOP 1k")
