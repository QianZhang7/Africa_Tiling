...
plot(density_by_population(world_2k_2010), type ="l", col = "red", main = "Pdf for 2km grids(aggregated)", xlab = 'population counts',  xaxt='n', xlim = c(0,5), ylim= c(0,9))
lines(density_by_population(mcdi_2k_2010), col = 'orange')
ticks <- seq(0, 5, by=1)
labels <- sapply(ticks, function(i) as.expression(bquote(10^ .(i))))
axis(1, at=c(0, 1,2,3,4,5), labels=labels)
legend(3.8,8,c("WorldPOP:2km", "mcdi:2km"), col = c("red", "orange"), lty = 1, cex = 0.6)
...
plot(density_by_population(world_5k_2010), type ="l", col = "red", main = "Pdf for 5km grids(aggregated)", xlab = 'population counts',  xaxt='n', xlim = c(0,5), ylim= c(0,9))
lines(density_by_population(mcdi_5k_2010), col = 'orange')
ticks <- seq(0, 5, by=1)
labels <- sapply(ticks, function(i) as.expression(bquote(10^ .(i))))
axis(1, at=c(0, 1,2,3,4,5), labels=labels)
legend(3.8,8,c("WorldPOP:2km", "mcdi:2km"), col = c("red", "orange"), lty = 1, cex = 0.6)
